...
const FURNITURE_CATEGORIES = ["living_room", "bedroom", "dining", "office", "outdoor", "decor", "packages", "furniture", "pillows_bedding"];
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/products?limit=500`)
      ...
      .then((data) => {
        ...
        const products = allProducts.filter((p) => FURNITURE_CATEGORIES.includes(p.category));

        // المنتجات المميزة من غرفة المعيشة وغرفة النوم
        setFeatured(
          allProducts
            .filter((p) => p.category === "living_room" || p.category === "bedroom")
            .slice(0, 6)
        );
